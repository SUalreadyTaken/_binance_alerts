...
public class CheckPrice {
    ...
    synchronized (watchlist.getPricesWatchlist()) {
      ...
      while (watchListIterator.hasNext()) {
        ...
        if (candle != null) {
          ...
          while (it.hasNext()) {
            ...
            if (isBetween(candle.getLow(), candle.getHigh(), alert.getPrice())) {
              ...
              if (candle.getOpen() < candle.getClose()) {
                ...
                text = "🚨📈 " + entry.getKey().label + " rose above " + alert.getPrice() + " " + alert.getMsg();
              } else {
                // Price is falling
                text = "🚨📉 " + entry.getKey().label + " fell below " + alert.getPrice() + " " + alert.getMsg();
              }
              try {
                System.out.println("id: " + alert.getChatId() + " msg: " + text);
          ...
          if (removeFromDB != null) {
            try {
              dbQueue.getDbQueue().put(new DbInput(DBCommand.REMOVE_BINANCE, entry.getKey(), convertFromChatAlert(removeFromDB)));
            } catch (Exception e) {
            }
          }
          watchlist.modifyWatchList();
          try {
            long timeToSleep = (500 - (System.currentTimeMillis() - startTime));
            if(timeToSleep > 0)
  ...
  private Map<String, List<Float>> convertFromChatAlert(List<ChatAlert> chatAlertList) {
    Map<String, List<Float>> result = new HashMap<>();
    for (ChatAlert ca : chatAlertList) {
      if (!result.containsKey(ca.getChatId())) {
        List<Float> priceList = chatAlertList.stream().filter(c -> c.getChatId().equals(ca.getChatId()))
