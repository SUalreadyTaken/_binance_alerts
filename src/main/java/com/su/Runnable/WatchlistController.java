  ...
  private void getWatchList(TelegramInput message) {
    try {
      ...
      if (symbol != null) {
        ...
        if (coinList != null) {
          coinList.sort(Comparator.comparing(WatchlistCoin::getPrice));
        ...
        if (cca != null) {
          ...
          for (CoinAlert ca : coinAlertList) {
            ...
            ca.getPriceInfoList().sort(Comparator.comparing(PriceInfo::getPrice));
            ...
            if (isAboveZero(wantedPrice, respondMessage) && isBetweenRange(wantedPrice, respondMessage, currentPrice)) {
              if (coinList != null) {
                ...
                for (int i = 0; i < coinList.size(); i++) {
                  if (coinList.get(i).getChatId().equals(message.getChatId()) && coinList.get(i).getPrice() == wantedPrice) {
                    found = true;
                    break;
                  }
                }
                if (!found) {
                  ...
                  respondMessage.append("✔️ Added ").append(symbol).append(" ").append(wantedPrice).append(" ")
    ...
    try {
      ...
      if (symbol != null) {
        ...
        if (message.getMsgList().size() == 3) {
          ...
          if (coinList != null) {
            for (int i = 0; i < coinList.size(); i++) {
              if (coinList.get(i).getChatId().equals(message.getChatId())
            ...
            respondMessage.append("✔️ Added ").append(symbol).append(" ").append(pricesForEdit.get(1))
                .append(" to watchlist\n");
            try {
              ...
              .forEach(f -> {
                if (coinList == null) {
                  try {
                    ...
                    respondMessage.append("✔️ Added ").append(symbol).append(" ").append(f).append(" to watchlist\n");
                    pricesToAdd.add(f);
                  } catch (Exception e) {
                  ...
                  boolean found = false;
                  for (int i = 0; i < coinList.size(); i++) {
                    if (coinList.get(i).getChatId().equals(message.getChatId()) && coinList.get(i).getPrice() == f) {
                      found = true;
                      break;
                    }
                  }
