...
public class ChatCoinAlertService {
  ...
  public void removeMultiplePricesFromChatsAlertList(String chatId, CoinSymbol symbol, List<PriceInfo> removePriceList) {
  ...
  public void addPriceToChatsAlertList(String chatId, CoinSymbol symbol, float price, String msg) {
    Optional<ChatCoinAlert> inCollection = chatCoinAlertsRepository.findFirstByChatId(chatId);
   ...
   */
  public void addMultiplePricesToChatsAlertList(String chatId, CoinSymbol symbol, List<PriceInfo> addPriceList) {
    Optional<ChatCoinAlert> inCollection = chatCoinAlertsRepository.findFirstByChatId(chatId);
  ...
  public void removePricesFormSameSymbol(CoinSymbol symbol, Map<String, List<Float>> priceMap) {
    ...
    if (!inCollection.isEmpty()) {
      ...
      for (Entry<String, List<Float>> entry : priceMap.entrySet()) {
        Optional<List<CoinAlert>> thisChatsCoinAlertList = inCollection.stream()
            .filter(c -> c.getChatId().equals(entry.getKey())).findFirst().map(ca -> ca.getCoinAlertList());
        if (thisChatsCoinAlertList.isPresent()) {
          ...
          if (thisChatsCoinAlert.isPresent()) {
            if (thisChatsCoinAlert.get().getPriceInfoList().removeIf(c -> entry.getValue().contains(c.getPrice()))) {
              ...
              needToSave.add(inCollection.stream().filter(c -> c.getChatId().equals(entry.getKey())).findFirst().get());
            }
          }
        }
  ...
  public ChatCoinAlert getFirstByChatId(String chatId) {
    Optional<ChatCoinAlert> inCollection = chatCoinAlertsRepository.findFirstByChatId(chatId);
  ...
  public void removePricesFormChatsAlertLists(List<ChatCoinAlert> toRemoveList) {
    List<String> idList = toRemoveList.stream().map(ChatCoinAlert::getChatId).collect(Collectors.toList());
    ...
    if (!collectionList.isEmpty()) {
      collectionList.forEach(inC -> {
        boolean needToSave = false;
        Optional<ChatCoinAlert> cca = toRemoveList.stream().filter(c -> c.getChatId().equals(inC.getChatId())).findFirst();
        if (cca.isPresent()) {
  ...
  public void addPricesToChatsAlertLists(List<ChatCoinAlert> toAddList) {
    List<String> idList = toAddList.stream().map(ChatCoinAlert::getChatId).collect(Collectors.toList());
    Pageable tmpPageable = PageRequest.of(0, idList.size());
    List<ChatCoinAlert> collectionList = new ArrayList<>(
        chatCoinAlertsRepository.findByChatIdIn(idList, tmpPageable).getContent());
    ...
    if (!collectionList.isEmpty()) {
      collectionList.forEach(inC -> {
        boolean needToSave = false;
        Optional<ChatCoinAlert> cca = toAddList.stream().filter(c -> c.getChatId().equals(inC.getChatId())).findFirst();
        if (cca.isPresent()) {
          List<CoinAlert> coinAlertList = cca.get().getCoinAlertList();
          for (int i = 0; i < coinAlertList.size(); i++) {
            for (int j = 0; j < inC.getCoinAlertList().size(); j++) {
    ...
    if (collectionList.size() != toAddList.size()) {
      for (int i = 0; i < toAddList.size(); i++) {
        ...
        for (int j = 0; j < collectionList.size(); j++) {
          if (collectionList.get(j).getChatId().equals(toAddList.get(i).getChatId())) {
            brandNewChat = false;
            break;
          }
        }
   ...
   * Deprecated
   */
  public void removePriceFromChatsAlertList(String chatId, CoinSymbol symbol, float price) {
    Optional<ChatCoinAlert> inCollection = chatCoinAlertsRepository.findFirstByChatId(chatId);
    if (inCollection.isPresent()) {
      for (int i = 0; i < inCollection.get().getCoinAlertList().size(); i++) {
        if (inCollection.get().getCoinAlertList().get(i).getSymbol().equals(symbol)) {
